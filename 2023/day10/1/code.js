...
const input_test3 = `..F7.
.FJ|.
SJ.L7
|F--J
LJ...`

const input_test4 = `7-F7-
.FJ|7
SJLL7
|F--J
LJ.LJ`
...
function calc(input){
    data = input.split('\n');
    let start_pos_neighbors = get_start_pos_neighbors();

    //* start_pos_neighbors.forEach(start_pos => {
        //* pos = start_pos;
        pos = start_pos_neighbors[0];
        pos_before = get_pos_s();
        let count;
        for (count = 0; data[pos[1]][pos[0]] != 'S'; count++) {
            console.log(" ")
            console.log(pos_before, pos)
            console.log(data[pos[1]][pos[0]])

            c_now = data[pos[1]][pos[0]];
            for (let index = 0; index < 4; index++) {
                if(index == 0){
                    ...
                    if(x != pos_before[0] || y != pos_before[1]){
                        c_new = data[y][x];
                        console.log(index, c_new, x, y)
                        if(['|', 'L', 'J'].includes(c_now) && ['|', '7', 'F', 'S'].includes(c_new)){
                            pos_before = pos;
                            pos = [x, y];
                            break;
                        }
                    ...
                    if(x != pos_before[0] || y != pos_before[1]){
                        c_new = data[y][x];
                        console.log(index, c_new, x, y)
                        if(['-', 'L', 'F'].includes(c_now) && ['-', 'J', '7', 'S'].includes(c_new)){
                            pos_before = pos;
                            pos = [x, y];
                            break;
                        }
                    ...
                    if(x != pos_before[0] || y != pos_before[1]){
                        c_new = data[y][x];
                        console.log(index, c_new, x, y)
                        if(['|', '7', 'F'].includes(c_now) && ['|', 'L', 'J', 'S'].includes(c_new)){
                            pos_before = pos;
                            pos = [x, y];
                            break;
                        }
                    ...
                    if(x != pos_before[0] || y != pos_before[1]){
                        c_new = data[y][x];
                        console.log(index, c_new, x, y)
                        if(['-', 'J', '7'].includes(c_now) && ['-', 'L', 'F', 'S'].includes(c_new)){
                            pos_before = pos;
                            pos = [x, y];
                            break;
                        }
...
function get_start_pos_neighbors(){
    let pos_s = get_pos_s();

    let start_pos_neighbors = [];

    for (let index = 0; index < 4; index++) { // Die Nachbarn von S prüfen
        if(index == 0){
            dx = 0;
            dy = -1;
        } else if(index == 1){
        ...
        connected = false;
        if(index == 0 && ['|', '7', 'F'].includes(c)){
            connected = true;
        } else if(index == 1 && ['-', 'J', '7'].includes(c)){
            connected = true;
        } else if(index == 2 && ['|', 'L', 'J'].includes(c)){
            connected = true;
        } else if(index == 3 && ['-', 'L', 'F'].includes(c)) {
            connected = true;
        }

        if(connected){
            start_pos_neighbors.push([x, y])
        }
    
    }
    return start_pos_neighbors
}


function get_pos_s(){
    for (let index = 0; index < data.length; index++) {
        if(data[index].split('S').length > 1){
            y = index;
            x = data[index].split('S')[0].length;
            return [x, y];
        }
    }
}
