...
function clac(input){
    const data = sort_data(input);

    sum = 0;
    for (const id in data) {
        red = Number(data[id]["red"]);
        green = Number(data[id]["green"]);
        blue = Number(data[id]["blue"]);
        sum += red * green * blue;
    }
...
function sort_data(input){
    ...
    games = input.split("Game");
    games.shift();
    games.forEach(game => {        
        game_array = game.split(":");
        id = game_array.shift().replace(/ /gi, "");
        data[id] = {};
        game_array.forEach(hands => {
            hands.split(";").forEach(hand => {
                hand = hand.replace(/ /gi, "");
                hand = hand.replace(/\n/gi, "");
                hand.split(",").forEach(type => {
                    count = type.replace(/red/gi, "");
                    count = count.replace(/green/gi, "");
                    count = count.replace(/blue/gi, "");
                    count = Number(count)

                    if(type.at(-1) == 'd'){color = 'red';}
                    else if(type.at(-1) == 'n'){color = 'green';}
                    else if(type.at(-1) == 'e'){color = 'blue';}

                    if(!data[id][color] || data[id][color] < count){data[id][color] = count;}
                });
            });
        });
    });
